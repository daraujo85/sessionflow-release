"""Integration tests for session lifecycle endpoints (TMUX-09/10/11).

Covers:
- ``DELETE /sessions/{id}``  -> ``kill`` command
- ``PATCH /sessions/{id}``   -> ``rename`` command
- ``POST /sessions/{id}/resume`` -> ``resume`` command
...
async def test_kill_publishes_command(settings, drain_commands):
...
@pytest.mark.integration
async def test_rename_publishes_command(settings, drain_commands):
    old_name = f"old-{uuid.uuid4().hex[:8]}"
    new_name = f"new-{uuid.uuid4().hex[:8]}"
    session_id = await _seed(settings, "running", old_name)

    app = create_app(settings=settings)
    async with await _client(app) as client:
        async with app.router.lifespan_context(app):
            resp = await client.patch(
                f"/sessions/{session_id}", json={"new_name": new_name}
            )
    ...
    assert msg["payload"] == {"old": old_name, "new": new_name}


@pytest.mark.integration
async def test_rename_accepts_name_alias(settings, drain_commands):
    old_name = f"old-{uuid.uuid4().hex[:8]}"
    new_name = f"new-{uuid.uuid4().hex[:8]}"
    session_id = await _seed(settings, "running", old_name)
    ...
    async with await _client(app) as client:
        async with app.router.lifespan_context(app):
            resp = await client.patch(
                f"/sessions/{session_id}", json={"name": new_name}
            )
...
@pytest.mark.integration
async def test_rename_empty_name_unprocessable(settings):
    session_id = await _seed(settings, "running", f"old-{uuid.uuid4().hex[:8]}")

    app = create_app(settings=settings)
    async with await _client(app) as client:
        async with app.router.lifespan_context(app):
            resp = await client.patch(f"/sessions/{session_id}", json={"name": "  "})
    assert resp.status_code == 422


@pytest.mark.integration
async def test_resume_publishes_command(settings, drain_commands):
    name = f"res-{uuid.uuid4().hex[:8]}"
    session_id = await _seed(settings, "stopped", name)
...
async def test_kill_missing_not_found(settings):
    ...
    async with await _client(app) as client:
        async with app.router.lifespan_context(app):
            resp = await client.delete(f"/sessions/{missing_id}")
    assert resp.status_code == 404


@pytest.mark.integration
async def test_rename_missing_not_found(settings):
    app = create_app(settings=settings)
    missing_id = str(ObjectId())
    async with await _client(app) as client:
        async with app.router.lifespan_context(app):
            resp = await client.patch(
                f"/sessions/{missing_id}", json={"name": "whatever"}
            )
    assert resp.status_code == 404


@pytest.mark.integration
async def test_resume_missing_not_found(settings):
    app = create_app(settings=settings)
    missing_id = str(ObjectId())
    async with await _client(app) as client:
        async with app.router.lifespan_context(app):
            resp = await client.post(f"/sessions/{missing_id}/resume")
    assert resp.status_code == 404
